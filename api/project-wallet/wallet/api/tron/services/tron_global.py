...
USDT_CONTRACT_ADDRESS = 'TKmCGCuVpW9QQzdovLdo8NPCMrvbkFMh9h'

# API ключ от TronGrid
TRON_GRID_API_KEY = '8e27a11e-9607-4653-b814-2fa47293e7c9'
...
class TRCToken:
    ...
    def transfer_trc20(self, from_address, private_key, to_address, amount):
        """Отправить весь баланс trc20 на указанный адрес"""
        sender_key = PrivateKey(bytes.fromhex(private_key))
        balance = int(amount * (10**6))
        if balance == 0:
            return False, False
        contract = self.tron.get_contract(USDT_CONTRACT_ADDRESS)
        contract.abi = ABI
